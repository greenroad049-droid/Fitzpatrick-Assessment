...
def add_keyboard_shortcuts():
    ...
    components.html(
        ...
        (function() {
            ...
            function clickButtonByPartialLabel(text) {
                const buttons = doc.querySelectorAll('button');
                for (const btn of buttons) {
            ...
            doc.addEventListener('keydown', function(e) {
                ...
                if (e.key === 'Backspace') {
                    e.preventDefault();
                    clickButtonByPartialLabel('Back');
                }
            });
        })();
        </script>
...
def get_sheet():
    ...
    if not all_values:
        # Empty sheet: create the header row.
        sheet.append_row(SHEET_HEADER)
    elif all_values[0] != SHEET_HEADER and len(all_values) == 1:
        # Only a (wrong/leftover) header row and no data yet: safe to fix it.
...
def admin_panel():
    ...
    missing_cols = [c for c in SHEET_HEADER if c not in raw_df.columns]
    if missing_cols:
        st.error(
            "Your Google Sheet's header row doesn't match what the app expects "
            f"(missing column(s): {', '.join(missing_cols)})."
        )
        st.info(
            "Fix: open your Google Sheet and make sure **row 1** contains exactly "
            "these column names, in this order:\n\n"
            f"`{' | '.join(SHEET_HEADER)}`\n\n"
            "Easiest fix: delete row 1 (or the whole sheet content) and reload this "
            "admin page — the app will recreate the correct header automatically."
        )
        st.dataframe(raw_df, use_container_width=True)
        st.stop()

    # If someone used "Back" to fix a misclick, the same evaluator+photo may
    # appear more than once. Keep only the latest (most recent timestamp)
    # answer for each pair — that's the corrected, final result.
    df = (
        raw_df.sort_values("timestamp")
        .drop_duplicates(subset=["evaluator", "photo"], keep="last")
        .reset_index(drop=True)
    )
    corrections = len(raw_df) - len(df)

    st.success(f"{len(df)} final evaluations from {df['evaluator'].nunique()} evaluator(s).")
    if corrections > 0:
        st.caption(f"({corrections} answer(s) were corrected using the Back button and excluded.)")
...
def evaluation_screen():
    ...
    if index > 0:
        if st.button("⬅ Back (fix last answer)", key=f"back_{index}"):
            st.session_state.index -= 1
            st.session_state.photo_start_time = time.time()
            st.rerun()
        st.caption("Tip: press Backspace to go back quickly.")
